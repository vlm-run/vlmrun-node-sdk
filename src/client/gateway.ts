...
const GATEWAY_TIMEOUT_FLOOR_MS = 600000;
...
export class Gateway {
  ...
  private _timeout(): number | undefined {
    ...
    if (timeout === DEFAULT_CLIENT_TIMEOUT_MS) {
      return GATEWAY_TIMEOUT_FLOOR_MS;
    }
    return timeout;
  }

  /**
   * Lazily-constructed OpenAI client pointed at the gateway.
   *
   ...
   *
   * @returns Promise<boolean> - true if the gateway is reachable and
  ...
  private async _healthViaModels(): Promise<boolean> {
    try {
      await this.models();
      return true;
    } catch {
